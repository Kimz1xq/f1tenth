...
is_simulation_workspace() {
  [[ -f /sim_ws/install/setup.bash ]]
}

terminate_simulation_cli_helpers() {
  local attempt
  local -a helper_pids=()

  # ros2cli's discovery daemon and interrupted inspection commands are not
  # members of the launch process group.  Inside the dedicated simulator
  # container they serve no hardware process, and retaining them can leave a
  # stale graph or Fast DDS port for the next run.  Never perform this cleanup
  # in the onboard workspace where hardware bringup is intentionally kept
  # alive across autonomy-controller restarts.
  is_simulation_workspace || return 0
  mapfile -t helper_pids < <(ps -eo pid=,args= | awk '
    $0 ~ /ros2cli\.daemon\.daemonize/ ||
    $0 ~ /\/opt\/ros\/humble\/bin\/ros2 (param|topic|service|lifecycle|node|doctor|run tf2_)/ {
      print $1
    }
  ')
  ((${#helper_pids[@]})) || return 0

  kill -TERM "${helper_pids[@]}" 2>/dev/null || true
  for attempt in {1..20}; do
    helper_pids=($(ps -o pid= -p "$(IFS=,; echo "${helper_pids[*]}")" \
      2>/dev/null))
    ((${#helper_pids[@]})) || return 0
    sleep 0.05
  done
  kill -KILL "${helper_pids[@]}" 2>/dev/null || true
}
...
# Make each simulator invocation independent even if a previous terminal was
# force-closed before its EXIT trap ran.
terminate_simulation_cli_helpers
...
# Direct discovery avoids a persistent ros2cli daemon in the simulator.  It is
# scoped to this managed shell and its helper commands, not global ROS nodes.
if is_simulation_workspace; then
